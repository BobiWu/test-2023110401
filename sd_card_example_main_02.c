...
//-------Ir RxNorth----------	
#define	Imm_IrRxNorthUnitTime	100
static  uint8_t	        RxNorthHighTimeCounter=0;	
static	uint8_t		    RxNorthLowTimeCounter=0;
static	uint8_t			RxNorthBitCounter=0;		
static	uint8_t			gettingRxNorth0or1=0;	
static	unsigned int 	gettingRxNorthData=0;	
static	unsigned int 	RxNorthGotData=0;	
static	uint8_t			gotRxNorth8BitDataHappen=0;
static	uint8_t			testRxNorth=0;
//-------Ir RxNorth----------

//-------Ir RxEast----------	
#define	Imm_IrRxEastUnitTime	100
static  uint8_t	        RxEastHighTimeCounter=0;	
static	uint8_t		    RxEastLowTimeCounter=0;
static	uint8_t			RxEastBitCounter=0;		
static	uint8_t			gettingRxEast0or1=0;	
static	unsigned int 	gettingRxEastData=0;	
static	unsigned int 	RxEastGotData=0;	
static	uint8_t			gotRxEast8BitDataHappen=0;
static	uint8_t			testRxEast=0;
//-------Ir RxEast----------

//-------Ir RxSouth----------	
#define	Imm_IrRxSouthUnitTime	100
static  uint8_t	        RxSouthHighTimeCounter=0;	
static	uint8_t		    RxSouthLowTimeCounter=0;
static	uint8_t			RxSouthBitCounter=0;		
static	uint8_t			gettingRxSouth0or1=0;	
static	unsigned int 	gettingRxSouthData=0;	
static	unsigned int 	RxSouthGotData=0;	
static	uint8_t			gotRxSouth8BitDataHappen=0;
static	uint8_t			testRxSouth=0;
//-------Ir RxSouth----------

//-------Ir RxWest----------	
#define	Imm_IrRxWestUnitTime	100
static  uint8_t	        RxWestHighTimeCounter=0;	
static	uint8_t		    RxWestLowTimeCounter=0;
static	uint8_t			RxWestBitCounter=0;		
static	uint8_t			gettingRxWest0or1=0;	
static	unsigned int 	gettingRxWestData=0;	
static	unsigned int 	RxWestGotData=0;	
static	uint8_t			gotRxWest8BitDataHappen=0;
static	uint8_t			testRxWest=0;
//-------Ir RxWest----------

//-------test ir detection--
static	uint8_t irBitSendingCounters = 29;	//32 //29 //28
#define	Imm_4LedsDelayTime	20500
static int  delay1000msAndTurnOffNorth4Led = 0;
static int  delay1000msAndTurnOffEast4Led = 0;
static int  delay1000msAndTurnOffSouth4Led = 0;
static int  delay1000msAndTurnOffWest4Led = 0;
//-------test ir detection--
...
#define IrRxNorth           GPIO_INPUT_IO_1
#define IrRxEast            GPIO_INPUT_IO_2
#define IrRxSouth           GPIO_INPUT_IO_3
#define IrRxWest            GPIO_INPUT_IO_4

#define MAIN_CUBE//MAIN_CUBE,SUB_CUBE1,SUB_CUBE2,SUB_CUBE2,SUB_CUBE3,SUB_CUBE4
/*
...
//-------Ir RxNorth----------
void IrRxNorthSub(void)
{
    int irRxNorth_state = 0;	    

    if(irBitSendingCounters > 0)
    {//if sending ir, do not rec
      	gettingRxNorth0or1 = 0;
      	RxNorthHighTimeCounter = 0;
      	RxNorthLowTimeCounter = 0;        
    }

    irRxNorth_state = gpio_get_level(IrRxNorth);	        	
    if(irRxNorth_state == 1)
    {//High come
        if(RxNorthLowTimeCounter < (1300/Imm_IrRxNorthUnitTime))//1000us
        {//;;checking data 0_1
            if(gettingRxNorth0or1 == 0)
            {//It is nosie,clear
                RxNorthLowTimeCounter = 0;
            }
            else
            {//;;
      	        RxNorthHighTimeCounter++;
      	        if(RxNorthHighTimeCounter >= (5500/Imm_IrRxNorthUnitTime))	
      	        {//more than 5.5ms if 3.4ms,error
      	            gettingRxNorth0or1 = 0;
      	            RxNorthHighTimeCounter = 0;
      	            RxNorthLowTimeCounter = 0;
                    RxNorthBitCounter = 0;
	  			    //if(RxNorthBitCounter==(32-8))//8
	  			    //{
	  			    //    gettingRxNorth0or1 = 0;
	  			    //    RxNorthHighTimeCounter = 0;
	  			    //    RxNorthLowTimeCounter = 0;
    			 	//    RxNorthGotData = gettingRxNorthData;
    			    //    gotRxNorth8BitDataHappen = 1;	//8bit
	  			    //}
      	        }
      	        else//;;
      	        {
	  			    if(RxNorthLowTimeCounter != 0)
	  			    {
	  			        if((RxNorthLowTimeCounter > (700/Imm_IrRxNorthUnitTime)) && (RxNorthLowTimeCounter < (1300/Imm_IrRxNorthUnitTime)))//1000us
	  			        {
    			            RxNorthLowTimeCounter = 0;
    			            gettingRxNorthData <<= 1;
	  			            if(RxNorthHighTimeCounter < (1500/Imm_IrRxNorthUnitTime))
	  			            {//;;Got 0
	  					        //gettingRxNorthData&=0x3FFFE;
	  						    gettingRxNorthData &= 0xFFFFFFFE;
	  			            }
	  			            else
	  			            {//;;Got 1
	  						    gettingRxNorthData |= 0x00000001;
	  			            }
	  			            RxNorthHighTimeCounter = 0;
	  			            RxNorthBitCounter--;
	  			            if(RxNorthBitCounter == 0)	
	  			            {//Finished
	  						    gettingRxNorth0or1 = 0;
	  						    RxNorthHighTimeCounter = 0;
	  						    RxNorthLowTimeCounter = 0;
	  						    gotRxNorth8BitDataHappen = 1;
	  						    RxNorthGotData = gettingRxNorthData;  
                                testRxNorth = 1;  			
                                if(RxNorthGotData == 0x00)//debug1
                                {
                                    RxNorthGotData = 0x58;
                                }
	  			            }
        ...
        {
            if(//666b
      	        ((5000/Imm_IrRxNorthUnitTime) <= RxNorthLowTimeCounter) &&
      	        (RxNorthLowTimeCounter < (8000/Imm_IrRxNorthUnitTime))
              )
            {//;;check header
                RxNorthBitCounter = 8;//32;//8;	
                gettingRxNorth0or1 = 1;
                gettingRxNorthData = 0;
                RxNorthHighTimeCounter = 0;
                RxNorthLowTimeCounter = 0;
            }
            else
            {
                RxNorthLowTimeCounter = 0;  	//;;notheader，not 0 or 1，error
            }
        }
    }
    else
    {//Low come
        RxNorthLowTimeCounter++;
    }
}
//-------Ir RxNorth----------

//-------Ir RxEast----------
void IrRxEastSub(void)
{
    int irRxEast_state = 0;	    

    if(irBitSendingCounters > 0)
    {//if sending ir, do not rec
      	gettingRxEast0or1 = 0;
      	RxEastHighTimeCounter = 0;
      	RxEastLowTimeCounter = 0;        
    }    

    irRxEast_state = gpio_get_level(IrRxEast);	        	
    if(irRxEast_state == 1)
    {//High come
        if(RxEastLowTimeCounter < (1300/Imm_IrRxEastUnitTime))//1000us
        {//;;checking data 0_1
            if(gettingRxEast0or1 == 0)
            {//It is nosie,clear
                RxEastLowTimeCounter = 0;
            }
            else
            {//;;
      	        RxEastHighTimeCounter++;
      	        if(RxEastHighTimeCounter >= (5500/Imm_IrRxEastUnitTime))	
      	        {//more than 5.5ms if 3.4ms,error
      	            gettingRxEast0or1 = 0;
      	            RxEastHighTimeCounter = 0;
      	            RxEastLowTimeCounter = 0;
                    RxEastBitCounter = 0;
	  			    //if(RxEastBitCounter==(32-8))//8
	  			    //{
	  			    //    gettingRxEast0or1 = 0;
	  			    //    RxEastHighTimeCounter = 0;
	  			    //    RxEastLowTimeCounter = 0;
    			 	//    RxEastGotData = gettingRxEastData;
    			    //    gotRxEast8BitDataHappen = 1;	//8bit
	  			    //}
      	        }
      	        else//;;
      	        {
	  			    if(RxEastLowTimeCounter != 0)
	  			    {
	  			        if((RxEastLowTimeCounter > (700/Imm_IrRxEastUnitTime)) && (RxEastLowTimeCounter < (1300/Imm_IrRxEastUnitTime)))//1000us
	  			        {
    			            RxEastLowTimeCounter = 0;
    			            gettingRxEastData <<= 1;
	  			            if(RxEastHighTimeCounter < (1500/Imm_IrRxEastUnitTime))
	  			            {//;;Got 0
	  					        //gettingRxEastData&=0x3FFFE;
	  						    gettingRxEastData &= 0xFFFFFFFE;
	  			            }
	  			            else
	  			            {//;;Got 1
	  						    gettingRxEastData |= 0x00000001;
	  			            }
	  			            RxEastHighTimeCounter = 0;
	  			            RxEastBitCounter--;
	  			            if(RxEastBitCounter == 0)	
	  			            {//Finished
	  						    gettingRxEast0or1 = 0;
	  						    RxEastHighTimeCounter = 0;
	  						    RxEastLowTimeCounter = 0;
	  						    gotRxEast8BitDataHappen = 1;
	  						    RxEastGotData = gettingRxEastData;  
                                testRxEast = 1;  			
                                if(RxEastGotData == 0x00)//debug1
                                {
                                    RxEastGotData = 0x58;
                                }                                
	  			            }
        ...
        {
            if(//666b
      	        ((5000/Imm_IrRxEastUnitTime) <= RxEastLowTimeCounter) &&
      	        (RxEastLowTimeCounter < (8000/Imm_IrRxEastUnitTime))
              )
            {//;;check header
                RxEastBitCounter = 8;//32;//8;	
                gettingRxEast0or1 = 1;
                gettingRxEastData = 0;
                RxEastHighTimeCounter = 0;
                RxEastLowTimeCounter = 0;
            ...
            {
                RxEastLowTimeCounter = 0;  	//;;notheader，not 0 or 1，error
            }
        }
    }
    else
    {//Low come
        RxEastLowTimeCounter++;
    }
}
//-------Ir RxEast----------

//-------Ir RxSouth----------
void IrRxSouthSub(void)
{
    int irRxSouth_state = 0;	    

    if(irBitSendingCounters > 0)
    {//if sending ir, do not rec
      	gettingRxSouth0or1 = 0;
      	RxSouthHighTimeCounter = 0;
      	RxSouthLowTimeCounter = 0;        
    }    

    irRxSouth_state = gpio_get_level(IrRxSouth);	        	
    if(irRxSouth_state == 1)
    {//High come
        if(RxSouthLowTimeCounter < (1300/Imm_IrRxSouthUnitTime))//1000us
        {//;;checking data 0_1
            if(gettingRxSouth0or1 == 0)
            {//It is nosie,clear
                RxSouthLowTimeCounter = 0;
            }
            else
            {//;;
      	        RxSouthHighTimeCounter++;
      	        if(RxSouthHighTimeCounter >= (5500/Imm_IrRxSouthUnitTime))	
      	        {//more than 5.5ms if 3.4ms,error
      	            gettingRxSouth0or1 = 0;
      	            RxSouthHighTimeCounter = 0;
      	            RxSouthLowTimeCounter = 0;
                    RxSouthBitCounter = 0;
	  			    //if(RxSouthBitCounter==(32-8))//8
	  			    //{
	  			    //    gettingRxSouth0or1 = 0;
	  			    //    RxSouthHighTimeCounter = 0;
	  			    //    RxSouthLowTimeCounter = 0;
    			 	//    RxSouthGotData = gettingRxSouthData;
    			    //    gotRxSouth8BitDataHappen = 1;	//8bit
	  			    //}
      	        }
      	        else//;;
      	        {
	  			    if(RxSouthLowTimeCounter != 0)
	  			    {
	  			        if((RxSouthLowTimeCounter > (700/Imm_IrRxSouthUnitTime)) && (RxSouthLowTimeCounter < (1300/Imm_IrRxSouthUnitTime)))//1000us
	  			        {
    			            RxSouthLowTimeCounter = 0;
    			            gettingRxSouthData <<= 1;
	  			            if(RxSouthHighTimeCounter < (1500/Imm_IrRxSouthUnitTime))
	  			            {//;;Got 0
	  					        //gettingRxSouthData&=0x3FFFE;
	  						    gettingRxSouthData &= 0xFFFFFFFE;
	  			            }
	  			            else
	  			            {//;;Got 1
	  						    gettingRxSouthData |= 0x00000001;
	  			            }
	  			            RxSouthHighTimeCounter = 0;
	  			            RxSouthBitCounter--;
	  			            if(RxSouthBitCounter == 0)	
	  			            {//Finished
	  						    gettingRxSouth0or1 = 0;
	  						    RxSouthHighTimeCounter = 0;
	  						    RxSouthLowTimeCounter = 0;
	  						    gotRxSouth8BitDataHappen = 1;
	  						    RxSouthGotData = gettingRxSouthData;  
                                testRxSouth = 1;  			
                                if(RxSouthGotData == 0x00)//debug1
                                {
                                    RxSouthGotData = 0x58;
                                }                                
	  			            }
        ...
        {
            if(//666b
      	        ((5000/Imm_IrRxSouthUnitTime) <= RxSouthLowTimeCounter) &&
      	        (RxSouthLowTimeCounter < (8000/Imm_IrRxSouthUnitTime))
              )
            {//;;check header
                RxSouthBitCounter = 8;//32;//8;	
                gettingRxSouth0or1 = 1;
                gettingRxSouthData = 0;
                RxSouthHighTimeCounter = 0;
                RxSouthLowTimeCounter = 0;
            }
            else
            {
                RxSouthLowTimeCounter = 0;  	//;;notheader，not 0 or 1，error
            }
        }
    }
    else
    {//Low come
        RxSouthLowTimeCounter++;
    }
}
//-------Ir RxSouth----------

//-------Ir RxWest----------
void IrRxWestSub(void)
{
    int irRxWest_state = 0;	    

    if(irBitSendingCounters > 0)
    {//if sending ir, do not rec
      	gettingRxWest0or1 = 0;
      	RxWestHighTimeCounter = 0;
      	RxWestLowTimeCounter = 0;        
    }

    irRxWest_state = gpio_get_level(IrRxWest);	        	
    if(irRxWest_state == 1)
    {//High come
        if(RxWestLowTimeCounter < (1300/Imm_IrRxWestUnitTime))//1000us
        {//;;checking data 0_1
            if(gettingRxWest0or1 == 0)
            {//It is nosie,clear
                RxWestLowTimeCounter = 0;
            }
            else
            {//;;
      	        RxWestHighTimeCounter++;
      	        if(RxWestHighTimeCounter >= (5500/Imm_IrRxWestUnitTime))	
      	        {//more than 5.5ms if 3.4ms,error
      	            gettingRxWest0or1 = 0;
      	            RxWestHighTimeCounter = 0;
      	            RxWestLowTimeCounter = 0;
                    RxWestBitCounter = 0;
	  			    //if(RxWestBitCounter==(32-8))//8
	  			    //{
	  			    //    gettingRxWest0or1 = 0;
	  			    //    RxWestHighTimeCounter = 0;
	  			    //    RxWestLowTimeCounter = 0;
    			 	//    RxWestGotData = gettingRxWestData;
    			    //    gotRxWest8BitDataHappen = 1;	//8bit
	  			    //}
      	        }
      	        else//;;
      	        {
	  			    if(RxWestLowTimeCounter != 0)
	  			    {
	  			        if((RxWestLowTimeCounter > (700/Imm_IrRxWestUnitTime)) && (RxWestLowTimeCounter < (1300/Imm_IrRxWestUnitTime)))//1000us
	  			        {
    			            RxWestLowTimeCounter = 0;
    			            gettingRxWestData <<= 1;
	  			            if(RxWestHighTimeCounter < (1500/Imm_IrRxWestUnitTime))
	  			            {//;;Got 0
	  					        //gettingRxWestData&=0x3FFFE;
	  						    gettingRxWestData &= 0xFFFFFFFE;
	  			            }
	  			            else
	  			            {//;;Got 1
	  						    gettingRxWestData |= 0x00000001;
	  			            }
	  			            RxWestHighTimeCounter = 0;
	  			            RxWestBitCounter--;
	  			            if(RxWestBitCounter == 0)	
	  			            {//Finished
	  						    gettingRxWest0or1 = 0;
	  						    RxWestHighTimeCounter = 0;
	  						    RxWestLowTimeCounter = 0;
	  						    gotRxWest8BitDataHappen = 1;
	  						    RxWestGotData = gettingRxWestData;  
                                testRxWest = 1;  			
                                if(RxWestGotData == 0x00)//debug1
                                {
                                    RxWestGotData = 0x58;
                                }                                                              
	  			            }
        ...
        {
            if(//666b
      	        ((5000/Imm_IrRxWestUnitTime) <= RxWestLowTimeCounter) &&
      	        (RxWestLowTimeCounter < (8000/Imm_IrRxWestUnitTime))
              )
            {//;;check header
                RxWestBitCounter = 8;//32;//8;	
                gettingRxWest0or1 = 1;
                gettingRxWestData = 0;
                RxWestHighTimeCounter = 0;
                RxWestLowTimeCounter = 0;
            ...
            {
                RxWestLowTimeCounter = 0;  	//;;notheader，not 0 or 1，error
            }
        }
    }
    else
    {//Low come
        RxWestLowTimeCounter++;
    }
}
//-------Ir RxWest----------
...
{
	if(delay1000msAndTurnOffNorth4Led == 1)
	{
		delay1000msAndTurnOffNorth4Led = 0;
		clear_north_4_led();
...
{
	int refreshLeds = 0;
	switch (RxNorthGotData){
	case 0xC8://(R)
			clear_north_4_led();
			led_strip_pixels_ShowRxData[1 * 3 + 0] = 0x08;
			led_strip_pixels_ShowRxData[1 * 3 + 1] = 0x00;    
			led_strip_pixels_ShowRxData[1 * 3 + 2] = 0x00; 
			delay1000msAndTurnOffNorth4Led = Imm_4LedsDelayTime;      
			refreshLeds = 1;			
	        break;
	case 0xA8://(G)
			clear_north_4_led();		
...
{
	if(delay1000msAndTurnOffEast4Led == 1)
	{
		delay1000msAndTurnOffEast4Led = 0;
		clear_east_4_led();
        ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
        ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));		
	}	
...
{
	int refreshLeds = 0;    
	switch (RxEastGotData){
	case 0xC8://(R)
			clear_east_4_led();
			led_strip_pixels_ShowRxData[13 * 3 + 0] = 0x08;
			led_strip_pixels_ShowRxData[13 * 3 + 1] = 0x00;
			led_strip_pixels_ShowRxData[13 * 3 + 2] = 0x00; 
			delay1000msAndTurnOffEast4Led = Imm_4LedsDelayTime;      
			refreshLeds = 1;        				                   	
	        break;                                 
	case 0xA8://(G)        
			clear_east_4_led();		                    
...
{
	if(delay1000msAndTurnOffSouth4Led == 1)
	{
		delay1000msAndTurnOffSouth4Led = 0;
		clear_south_4_led();
        ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
        ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));		
	}	
...
{
	int refreshLeds = 0;    
	switch (RxSouthGotData){
	case 0xC8://(R)
			clear_south_4_led();
		  led_strip_pixels_ShowRxData[47 * 3 + 0] = 0x08;
		  led_strip_pixels_ShowRxData[47 * 3 + 1] = 0x00;
		  led_strip_pixels_ShowRxData[47 * 3 + 2] = 0x00;                   	
		  delay1000msAndTurnOffSouth4Led = Imm_4LedsDelayTime;      
		  refreshLeds = 1;
	    break;                                 
	case 0xA8://(G)         
			clear_south_4_led();		                   
...
{
	if(delay1000msAndTurnOffWest4Led == 1)
	{
		delay1000msAndTurnOffWest4Led = 0;
		clear_west_4_led();
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));		
	}	
...
{
	int refreshLeds = 0;    
	switch (RxWestGotData){
	case 0xC8://(R)
			clear_west_4_led();
		  led_strip_pixels_ShowRxData[35 * 3 + 0] = 0x08;
		  led_strip_pixels_ShowRxData[35 * 3 + 1] = 0x00;
		  led_strip_pixels_ShowRxData[35 * 3 + 2] = 0x00;                   	
		  delay1000msAndTurnOffWest4Led = Imm_4LedsDelayTime;      
		  refreshLeds = 1;
	    break;                                 
	case 0xA8://(G)        
			clear_west_4_led();		                    
...
static bool IRAM_ATTR timer_group_isr_callback(void *args)
{
    ...
    */
	...
	if(delay1000msAndTurnOffNorth4Led > 1)
	{
		delay1000msAndTurnOffNorth4Led--;
	}	
	if(delay1000msAndTurnOffEast4Led > 1)
	{
		delay1000msAndTurnOffEast4Led--;
	}	
	if(delay1000msAndTurnOffSouth4Led > 1)
	{
		delay1000msAndTurnOffSouth4Led--;
	}	
	if(delay1000msAndTurnOffWest4Led > 1)
	{
		delay1000msAndTurnOffWest4Led--;
	}							
	//-------test ir detection--
...
//--------------
IR_TX_EAST  :   Header-6ms	    Bit1 Bit1 Bit0 Bit0     Bit0 Bit0 Bit0 Bit1     off         0b11000001      
...
IR_TX_NORTH :   Header-6ms	    Bit1 Bit1 Bit0 Bit0     Bit0 Bit0 Bit0 Bit1                 0b11001000      
  				111111    	    0001 0001 01   01       0001 01   01   01       0000		(0xFC454550)   
//--------------
IR_TX_EAST  :   Header-6ms	    Bit1 Bit1 Bit0 Bit0     Bit0 Bit0 Bit0 Bit1     off         0b10100001      
...
IR_TX_NORTH :   Header-6ms	    Bit1 Bit1 Bit0 Bit0     Bit0 Bit0 Bit0 Bit1                 0b10101000      
  				111111    	    0001 01   0001 01       0001 01   01   01       0000		(0xFC514550)
//--------------
IR_TX_EAST  :   Header-6ms	    Bit1 Bit1 Bit0 Bit0     Bit0 Bit0 Bit0 Bit1     off         0b10010001      
...
IR_TX_NORTH :   Header-6ms	    Bit1 Bit1 Bit0 Bit0     Bit0 Bit0 Bit0 Bit1                 0b10011000      
  				111111    	    0001 01   01   0001     0001 01   01   01       0000		(0xFC544550)
//--------------
IR_TX_EAST  :   Header-6ms	    Bit1 Bit1 Bit0 Bit0     Bit0 Bit0 Bit0 Bit1     off         0b01100001      
...
IR_TX_NORTH :   Header-6ms	    Bit1 Bit1 Bit0 Bit0     Bit0 Bit0 Bit0 Bit1                 0b01101000      
  				111111    	    01   0001 0001 01       0001 01   01   01       0000		(0xFD114550)
//--------------
IR_TX_EAST  :   Header-6ms	    Bit1 Bit1 Bit0 Bit0     Bit0 Bit0 Bit0 Bit1     off         0b01010001      
...
IR_TX_NORTH :   Header-6ms	    Bit1 Bit1 Bit0 Bit0     Bit0 Bit0 Bit0 Bit1                 0b01011000      
...
*/
    while (1) {  
        ...
        if (port0InputValue & port0StdbySwBit) {
            ...
            if (port0StdbySwOff_debounce >= 50) {
...
void SetAndWaitPlaySoundFile(const char * SoundName)
{
    ...
    else 
    {
        ...
        fread(wavBuffer, sizeof(char), (wavData_size % 1024), fname); // 读文件
...
void sound_terminate(void)
{
    vTaskDelay(200 / portTICK_PERIOD_MS);//vTaskDelay(pdMS_TO_TICKS(200)); //延时200ms
...
*/
    ...
    */
    ...
    led_strip_pixels_ShowRxData[1 * 3 + 1] = 0x08;    
    led_strip_pixels_ShowRxData[1 * 3 + 2] = 0x08; 
    //“RxNorthGotData” rec “NORTH” data	:	0xC8(R)        0xA8(G)        0x98(B)        0x68(RG)        0x58(RGB)
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);
		...
		led_strip_pixels_ShowRxData[2 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[2 * 3 + 2] = 0x08;				
		//“RxNorthGotData” rec “EAST” data	:	0xC1(R)        0xA1(G)        0x91(B)        0x61(RG)        0x51(RGB)	
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);    
		...
		led_strip_pixels_ShowRxData[3 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[3 * 3 + 2] = 0x08;				
		//“RxNorthGotData” rec “SOUTH” data	:	0xC4(R)        0xA4(G)        0x94(B)        0x64(RG)        0x54(RGB)
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);        
		...
		led_strip_pixels_ShowRxData[4 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[4 * 3 + 2] = 0x08;				
		//“RxNorthGotData” rec “WEST” data	:	0xC2(R)        0xA2(G)        0x92(B)        0x62(RG)        0x52(RGB)
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);          
		//--------    
    ...
    led_strip_pixels_ShowRxData[13 * 3 + 1] = 0x08;
    led_strip_pixels_ShowRxData[13 * 3 + 2] = 0x08;        
    //“RxEastGotData” rec “NORTH” data	:	0xC8(R)        0xA8(G)        0x98(B)        0x68(RG)        0x58(RGB)
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);
		...
		led_strip_pixels_ShowRxData[20 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[20 * 3 + 2] = 0x08;				
		//“RxEastGotData” rec “EAST” data		:	0xC1(R)        0xA1(G)        0x91(B)        0x61(RG)        0x51(RGB)	
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);    
		...
		led_strip_pixels_ShowRxData[27 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[27 * 3 + 2] = 0x08;				
		//“RxEastGotData” rec “SOUTH” data	:	0xC4(R)        0xA4(G)        0x94(B)        0x64(RG)        0x54(RGB)
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);        
		...
		led_strip_pixels_ShowRxData[34 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[34 * 3 + 2] = 0x08;				
		//“RxEastGotData” rec “WEST” data		:	0xC2(R)        0xA2(G)        0x92(B)        0x62(RG)        0x52(RGB)		
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);              
		//--------    
    ...
    led_strip_pixels_ShowRxData[47 * 3 + 1] = 0x08;
    led_strip_pixels_ShowRxData[47 * 3 + 2] = 0x08;        
    //“RxSouthGotData” rec “NORTH” data	:	0xC8(R)        0xA8(G)        0x98(B)        0x68(RG)        0x58(RGB)
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);
		...
		led_strip_pixels_ShowRxData[46 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[46 * 3 + 2] = 0x08;				
		//“RxSouthGotData” rec “EAST” data	:	0xC1(R)        0xA1(G)        0x91(B)        0x61(RG)        0x51(RGB)	
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);    
		...
		led_strip_pixels_ShowRxData[45 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[45 * 3 + 2] = 0x08;				
		//“RxSouthGotData” rec “SOUTH” data	:	0xC4(R)        0xA4(G)        0x94(B)        0x64(RG)        0x54(RGB)
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);        
		...
		led_strip_pixels_ShowRxData[44 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[44 * 3 + 2] = 0x08;				
		//“RxSouthGotData” rec “WEST” data	:	0xC2(R)        0xA2(G)        0x92(B)        0x62(RG)        0x52(RGB)		
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);         
		//--------    
    ...
    led_strip_pixels_ShowRxData[35 * 3 + 1] = 0x08;
    led_strip_pixels_ShowRxData[35 * 3 + 2] = 0x08;        
    //“RxWestGotData” rec “NORTH” data	:	0xC8(R)        0xA8(G)        0x98(B)        0x68(RG)        0x58(RGB)
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);
		...
		led_strip_pixels_ShowRxData[28 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[28 * 3 + 2] = 0x08;				
		//“RxWestGotData” rec “EAST” data		:	0xC1(R)        0xA1(G)        0x91(B)        0x61(RG)        0x51(RGB)	
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);    
		...
		led_strip_pixels_ShowRxData[21 * 3 + 1] = 0x08;
		led_strip_pixels_ShowRxData[21 * 3 + 2] = 0x08;				
		//“RxWestGotData” rec “SOUTH” data	:	0xC4(R)        0xA4(G)        0x94(B)        0x64(RG)        0x54(RGB)
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);        
		...
		led_strip_pixels_ShowRxData[14 * 3 + 1] = 0x08;		
		led_strip_pixels_ShowRxData[14 * 3 + 2] = 0x08;				
		//“RxWestGotData” rec “WEST” data		:	0xC2(R)        0xA2(G)        0x92(B)        0x62(RG)        0x52(RGB)		
    ESP_ERROR_CHECK(rmt_transmit(led_chan, led_encoder, led_strip_pixels_ShowRxData, sizeof(led_strip_pixels_ShowRxData), &tx_config));
    ESP_ERROR_CHECK(rmt_tx_wait_all_done(led_chan, portMAX_DELAY));
    vTaskDelay(300 / portTICK_PERIOD_MS);
		//--------    
...
//    gpio_isr_handler_add(GPIO_INPUT_IO_1, gpio_isr_handler, (void*) GPIO_INPUT_IO_1);    
    */
    ...
    {   
		...
		if(gotRxNorth8BitDataHappen == 1)          //gotRxNorth8BitDataHappen
		{
			gotRxNorth8BitDataHappen = 0;
            show_north_rec_data();
            printf("North-->%x\r\n", RxNorthGotData);//printf("Test IrRx\r\n");
		}	        
		if(gotRxEast8BitDataHappen == 1)          //gotRxEast8BitDataHappen  
		{
			gotRxEast8BitDataHappen = 0;
            show_east_rec_data();            
            printf("East-->%x\r\n", RxEastGotData);//printf("Test IrRx\r\n");
		}	       
		if(gotRxSouth8BitDataHappen == 1)           //gotRxSouth8BitDataHappen
		{
			gotRxSouth8BitDataHappen = 0;
            show_south_rec_data();                        
            printf("South-->%x\r\n", RxSouthGotData);//printf("Test IrRx\r\n");
		}	       
		if(gotRxWest8BitDataHappen == 1)        //gotRxWest8BitDataHappen
		{
			gotRxWest8BitDataHappen = 0;
            show_west_rec_data();                                    
            printf("West-->%x\r\n", RxWestGotData);//printf("Test IrRx\r\n");
		}	       
